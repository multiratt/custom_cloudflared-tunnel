...
if [ -n "$import_string" ]; then
  ...
  run_command chroot /tmp/cloudflared/ /usr/bin/cloudflared tunnel create cns-opnwrt
  echo "Tunnel created."
  echo ""
  ...
  echo "wget -qO- https://raw.githubusercontent.com/multiratt/custom_cloudflared-tunnel/main/cns.sh | ash -s -- --import=\"$tar_string\""
  echo ""
fi

echo "Starting cloudflared tunnel \"cns-opnwrt\"..."
chroot /tmp/cloudflared/ /usr/bin/cloudflared tunnel run cns-opnwrt > /dev/null 2>&1 &
