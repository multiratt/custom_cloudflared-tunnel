...
if [ -n "$import_string" ]; then
  ...
  run_command chroot /tmp/cloudflared/ /usr/bin/cloudflared tunnel create pks-opnwrt
  echo "Tunnel created."
  echo ""
  ...
  echo "wget -qO- /etc/custom/cloudflared/script.sh | ash -s -- --import=\"$tar_string\""
  echo ""
fi

echo "Starting cloudflared tunnel \"pks-opnwrt\"..."
chroot /tmp/cloudflared/ /usr/bin/cloudflared tunnel run pks-opnwrt > /dev/null 2>&1 &
